...
Usage:
    ./generate_grafana_live_stream.py <sensor_id> [grafana_host]

Example:
    ./generate_grafana_live_stream.py metsta155030
    ./generate_grafana_live_stream.py metsta155030 http://192.168.1.50:3000
"""
...
def generate_grafana_live_stream(sensor_id, grafana_host_arg=None):
    """
    ...
    # Determine Grafana host: use argument if provided, else localhost
    grafana_host = grafana_host_arg if grafana_host_arg else 'http://localhost:3000'
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        sys.stderr.write("Usage: ./generate_grafana_live_stream.py <sensor_id> [grafana_host]\n")
        sys.exit(1)

    sensor_id = sys.argv[1]
    # Check for optional second argument
    grafana_host_arg = sys.argv[2] if len(sys.argv) > 2 else None

    yaml_output = generate_grafana_live_stream(sensor_id, grafana_host_arg)
    if yaml_output:
        print(yaml_output)
