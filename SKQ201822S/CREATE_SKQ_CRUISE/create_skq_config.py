...
script to have it feed stored data to those ports:
  ...
  logger/utils/simulate_network.py  \
    --config local/sikuliaq/SKQ201822S/network_sim_SKQ201822S.yaml \
    --loop
    ...
    - class: ComposedWriter
      kwargs:
        transforms:
          ...
          kwargs:
            sensor_path: local/sikuliaq/sensor/*.yaml,local/sikuliaq/SKQ201822S/CREATE_SKQ_CRUISE/sensors.yaml
            sensor_model_path: local/sikuliaq/sensor_model/*.yaml,local/sikuliaq/SKQ201822S/CREATE_SKQ_CRUISE/sensor_models.yaml
            message_path: local/sikuliaq/message/*.yaml
